...
- [x] 딜러는 처음 카드 공개시 한 장만 공개한다.
- [x] 딜러와 플레이어 점수 비교하여 승자를 가린다.
- [x] 베팅금액이 주어진다면 베팅금액을 받는다.
- [x] 플레이어는 게임을 시작할 때 베팅 금액을 정해야 한다.
- [x] 처음 두장의 카드가 블랙잭을 경우, 베팅 금액의 1.5배를 딜러에게 받는다.
- [x] 딜러와 플레이어가 모두 동시에 블랙잭인 경우, 플레이어는 베팅한 금액을 돌려받는다.
- [x] 딜러와 플레이어가 모두 21을 초과하면 플레이어가 승리한다.
- [x] 승패 결정으로 딜러와 플레이어 수익이 결정된다.