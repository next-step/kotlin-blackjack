...
- [x] 입력 계층에선 배팅 금액을 입력받는다. 
  - [x] 숫자가 아닐 경우 메세지 출력 후 재시도 한다.
  - [x] 0 이상의 자연수가 아닌 경우 메세지 출력 후 재시도 한다.
- [x] 승패 계산 결과에 수익 결과도 추가된다. 
  - [x] 출력 계층에선 수익 결과를 출력한다.
- [x] 금액 객체가 생긴다
  - [x] 연산자 오버로딩으로 금액 더하기 빼기가 가능하다.
- [x] 수익 계산 로직이 추가된다.
  - [x] 카드를 추가로 뽑아 21점을 초과하면 베팅 금액을 전부 잃는다. (Burst)
  - [x] 처음 두 장의 합이 21이면 베팅 금액의 1.5배를 딜러에게 받는다.(BlackJack)
  - [x] 딜러와 플레이어 모두 블랙잭이면 금액을 돌려받는다.(Push)
  - [x] 딜러가 21점을 초과할 경우 게이머들은 승리 베팅 금액을 받는다.(Burst)
...
- Burst: 카드 총합이 21을 넘는 경우
- Push: 딜러와 카드가 동점인 경우 해당 프로젝트에선 양측이 모두 21점인 경우를 의미한다.
- Blackjack: 첫 2장의 합이 21점인경우
