...
class InputView(private val input: Input, private val output: Output) {
    val players: List<String> by lazy { askPlayers() }

    private fun askPlayers(): List<String> {
        output.print("게임에 참여할 사람의 이름을 입력하세요.(쉼표 기준으로 분리)")
        return input.readPlayers().split(",").map { it.trim() }
    }

    fun hasNextCard(playerName: String): Boolean {
        output.print("${playerName}는 한장의 카드를 더 받겠습니까?(예는 y, 아니오는 n)")
