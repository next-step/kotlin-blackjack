...
object OutputView {
    fun roundBeginNotice(status: List<PlayerStatus>) {
        val dealerStatus = status.find { it.name == "딜러" }
        check(dealerStatus != null) { "딜러는 비어있을 수 없습니다." }
        val playerNames = status.filter { it.name != "딜러" }.joinToString { it.name }
    ...
    fun gameResultNotice(players: List<PlayerStatus>) {
        println()
        players.forEach {
            println("${it.name}: ${it.gameResult}")
        }
    }
