...
sealed class CardHolder(val id: Int, val cardHand: CardHand, val name: String, val role: Role) {
    val isBust: Boolean
        get() = cardHand.totalScore > CardHand.BLACKJACK
    val winLoseDraw: WinLoseDraw = WinLoseDraw()
    ...
    class Player(id: Int, cardHand: CardHand, name: String) : CardHolder(id, cardHand, name, Role.PLAYER) {
        fun hitOrNot(askToPlayer: (name: String) -> PlayAnswer) = when (askToPlayer(name)) {
            PlayAnswer.Y -> true
            PlayAnswer.N -> false
        }
    }
