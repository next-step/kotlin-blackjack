...
## 리뷰 반영
- [x] NonEmptyList 로직을 Players 로 옮기기
- [x] Players 에 딜러는 제외하기
- [x] 딜러는 16 이하이면 반복적으로 뽑는것으로 고치기
- [x] ScoreCalculator 에서 indent 1개만 되도록 수정하기
- [x] CardName, ScoreName 은 view 패키지로 이동하기
- [x] test fixture 는 하단에 두도록 변경하기
- [x] Blackjack 은 컨트롤러 이므로 view 객체에 의존하도록 변경
- [x] Fetcher 와 Printer 는 invoke operator 를 사용하도록 변경
- [x] 플레이어 관련 함수는 Players 일급 컬렉션에게 위임하도록 변경 
...
## To Do
- [x] 각 플레이어는 베팅 금액을 지정할 수 있다
  - [x] 베팅 금액은 0보다 커야한다
- [x] 게임 결과에 따라 베팅 금액을 얼마만큼 돌려받을 수 있는지 알 수 있다 (ProfitCalculator)
  - [x] 플레이어와 딜러가 모두 블랙잭인 경우 플레이어는 베팅한 금액을 돌려받는다
  - [x] 플레이어가 블랙잭, 딜러가 아닐 경우 플레이어는 베팅한 금액의 1.5 배를 추가로 받는다
  - [x] 플레이어가 버스트일 경우 딜러 결과에 상관 없이 베팅한 금액을 잃는다
  - [x] 플레이어는 21이하, 딜러는 버스트일 경우 베팅한 금액만큼 추가로 받는다
  - [x] 플레이어는 21이하, 딜러는 블랙잭일 경우 베팅한 금액을 잃는다
  - [x] 플레이어가 딜러보다 높을 경우 베팅한 금액만큼 추가로 받는다
  - [x] 플레이어가 딜러와 같을 경우 베팅한 금액을 돌려받는다
  - [x] 플레이어가 딜러보다 낮을 경우 베팅한 금액을 잃는다
- [x] 총 돌려받은 베팅 금액을 프린트 할 수 있다