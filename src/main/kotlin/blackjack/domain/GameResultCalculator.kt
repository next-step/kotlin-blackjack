...
object GameResultCalculator {
    ...
    private fun dealerLoseResult(game: Game): BlackJackResult {
        val playerResults = game.players.map { player ->
            PlayerResult(
    ...
    private fun compareGameResults(game: Game, dealerScore: Int): BlackJackResult {
        val playerResults = game.players.map { player ->
            val playerScore = player.getPlayerBlackJack().getTotalCardValue()
            when {
                playerScore > BlackJack.BUST_LIMIT_VALUE -> PlayerResult(
                    playerName = player.name,
                    winCount = 0,
                    loseCount = 1,
                    drawCount = 0
