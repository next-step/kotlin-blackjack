...
- [x] 플레이어는 시작(Start) 상태에서 승패를 조회하면 UnsupportedOperationException 이 발생한다.
- [x] 플레이어는 힛(Hit) 상태에서 승패를 조회하면 UnsupportedOperationException 이 발생한다.
- [x] 플레이어와 딜러의 상태와 카드 숫자 총합을 비교해서 승패를 가린다.
- [x] 플레이어는 상태의 승패 결과에 따라 최종 수익을 계산한다.
...
- [x] 게임을 시작하면 플레이어는 두 장의 카드를 지급 받으며, 두 장의 카드 숫자를 합쳐 21을 초과하지 않으면서 21에 가깝게 만들면 이긴다. 21을 넘지 않을 경우 원한다면 얼마든지 카드를 계속 뽑을 수 있다. 단, 카드를 추가로 뽑아 21을 초과할 경우 베팅 금액을 모두 잃게 된다.
...
- [x] 게임을 완료한 후 각 플레이어별로 최종수익을 출력한다.
