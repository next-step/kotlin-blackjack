...
class BlackjackTest: FunSpec({
    ...
    test("참가자 [pobi, jason]에게 각각 [2하트, 8스페이드]와 [7클로버, K스페이드]를 나누어 줄 경우 pobi = [2하트, 8스페이드], jason = [7클로버, K스페이드]를 가지고 있어야 한다.") {
        ...
        user1.cards + Card.from("하트","2")
        user1.cards + Card.from("스페이드", "8")
        user1.cards.cards[0] shouldBe Card.from("하트", "2")
        user1.cards.cards[1] shouldBe Card.from("스페이드", "8")

        val user2 = User.from("jason")
        user2.cards + Card.from("클로버", "7")
        user2.cards + Card.from("스페이드", "K")
        user2.cards.cards[0] shouldBe Card.from("클로버", "7")
        user2.cards.cards[1] shouldBe Card.from("스페이드", "K")
    }

    test("참가자 [pobi]가 [K하트, 4스페이드] 가지고 있는 상태에서 [5스페이드] 한장 더 받을 경우 카드가 한장 더 늘어나야 한다.") {
        val user = User.from("pobi")
        user.cards + Card.from("하트", "K")
        user.cards + Card.from("스페이드", "4")
        user.cards + Card.from("스페이드", "5")
        user.cards.cards.size shouldBe 3
    }
    ...
    test("참가자 [pobi]가 [K하트, 4스페이드]를 가지고 있을 경우 카드를 더 받을 수 있다.") {
        ...
        user.cards + Card.from("하트", "K")
        user.cards + Card.from("스페이드", "4")
        (user.cards.getMinTotalSum() < 21) shouldBe true
    }
    ...
    test("참가자 [pobi]가 [K하트, 4스페이드, 7클로버]를 가지고 있을 경우 카드를 더 받을 수 없다.") {
        ...
        user.cards + Card.from("하트", "K")
        user.cards + Card.from("스페이드", "4")
        user.cards + Card.from("클로버", "7")
        (user.cards.getMinTotalSum() < 21) shouldBe false
    }
