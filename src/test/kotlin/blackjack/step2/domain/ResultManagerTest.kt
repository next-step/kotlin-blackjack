...
class ResultManagerTest : FunSpec({
    ...
    test("딜러와 플레이어 모두 블랙잭이면 플레이어는 배팅금액을 돌려받는다. (수익은 0원)") {
        ...
        val player = Player("dongyeon", createCards(CardNumber.ACE, CardNumber.KING), bet = 10000)
        val participants = listOf(dealer, player)

        // when
        val results = ResultManager.calculateFinalProfit(participants)
    ...
    test("플레이어 버스트이면 플레이어는 배팅금액 전액을 잃는다.") {
        ...
        val player =
            Player("dongyeon", createCards(CardNumber.TEN, CardNumber.JACK, CardNumber.TWO), bet = 10000) // 22점
        val participants = listOf(dealer, player)
    ...
    test("딜러 버스트이면 플레이어들은 배팅금액만큼 수익을 얻는다.") {
        ...
        val player1 = Player("dongyeon", createCards(CardNumber.EIGHT, CardNumber.TEN), bet = 10000) // 18점
        val player2 = Player("pobi", createCards(CardNumber.KING, CardNumber.QUEEN), bet = 20000) // 20점
    ...
    test("플레이어 점수가 딜러보다 높으면 플레이어는 배팅금액만큼 얻는다.") {
        ...
        val player = Player("dongyeon", createCards(CardNumber.KING, CardNumber.FIVE), bet = 10000) // 15점
        val participants = listOf(dealer, player)
    ...
    test("플레이어 점수가 딜러보다 낮으면 플레이어는 배팅금액만큼 잃는다.") {
        // given
        val dealer = Dealer(cards = createCards(CardNumber.TEN, CardNumber.NINE)) // 19점
        val player = Player("dongyeon", createCards(CardNumber.TEN, CardNumber.EIGHT), bet = 10000) // 18점
        val participants = listOf(dealer, player)
    ...
    test("플레이어와 딜러 점수가 같으면 수익은 0이다.") {
        ...
        val player = Player("dongyeon", createCards(CardNumber.KING, CardNumber.QUEEN), bet = 10000) // 20점
        val participants = listOf(dealer, player)
    ...
    test("여러 플레이어가 있을 때 최종 딜러 수익은 모든 플레이어 수익의 합의 음수다.") {
        ...
        val player1 = Player("dongyeon", createCards(CardNumber.NINE, CardNumber.EIGHT), bet = 10000) // 17점
        val player2 =
            Player("pobi", createCards(CardNumber.TWO, CardNumber.TEN, CardNumber.JACK), bet = 20000) // 22점(bust)
        val player3 = Player("jason", createCards(CardNumber.ACE, CardNumber.KING), bet = 30000) // 블랙잭
