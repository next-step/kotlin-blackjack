...
BetCalculatorTest : StringSpec({
    "딜러가 승리할 경우 딜러가 배팅 금액을 가진다" {
        ...
        BetCalculator.settleBetMoney(WinType.DEALER_WIN, player, dealer)
    ...
    "플레이어가 승리할 경우 플레이어에게 배팅 금액을 가진다" {
        ...
        BetCalculator.settleBetMoney(WinType.PLAYER_WIN, player, dealer)
        dealer.wallet().balance() shouldBe -50_000
        dealer.wallet().income() shouldBe -50_000
    ...
    "딜러가 Bust 된다면(패가 21 초과) 그 시점 까지 남아있던 플레이어만 승리한다." {
        ...
        BetCalculator.settleBetMoney(WinType.DEALER_BUST, blackJacked, dealer)
    ...
    "플레이어가 처음 뽑은 2자리의 값이 블랙잭(21)이라면 1.5배를 돌려받는다." {
        ...
        BetCalculator.settleBetMoney(WinType.PLAYER_BLACK_JACK, player, dealer)
        dealer.wallet().balance() shouldBe -5_000
