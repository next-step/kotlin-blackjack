...
class ScoreTest {
    ...
    fun `Score는 더할 수 있다`(score1: Int, score2: Int, sum: Int) {
        val firstScore = Score.from(score1)
        val secondScore = Score.from(score2)
        assertThat(firstScore + secondScore).isEqualTo(Score.from(sum))
    }
    ...
    fun `Score는 Bust를 알 수 있다`(score: Int, expected: Boolean) {
        val firstScore = Score.from(score)
