...
class GamblerTest : StringSpec({
    ...
    "카드들의 점수 합이 1 ~ 20점 이내면 겜블러는 카드를 뽑을 수 있다." {
        Sign.values().forEach {
            val gambler = Gambler.from(
                Name.from("aaa"),
                Cards.from(setOf(Card.of(Pattern.SPACE, Sign.NINE), Card.of(Pattern.HEART, it)))
            )
    ...
    "겜블러는 버스트가 안되고, 딜러가 버스트가 되면 겜블러가 이긴다." {
        val gambler = Gambler.from(
            Name.from("aaa"),
            Cards.from(setOf(Card.of(Pattern.HEART, Sign.SEVEN), Card.of(Pattern.CLOVER, Sign.SEVEN)))
        )
        val dealer = Dealer.from(
            Name.from("딜러"),
            Cards.from(
    ...
    "겜블러와 딜러가 버스트가 안되고, 겜블러의 숫자가 높을 경우 겜블러가 이긴다." {
        val gambler = Gambler.from(
            Name.from("aaa"),
            Cards.from(setOf(Card.of(Pattern.HEART, Sign.EIGHT), Card.of(Pattern.CLOVER, Sign.SEVEN)))
        )
        val dealer = Dealer.from(
            Name.from("딜러"),
            Cards.from(setOf(Card.of(Pattern.HEART, Sign.SEVEN), Card.of(Pattern.CLOVER, Sign.SEVEN)))
