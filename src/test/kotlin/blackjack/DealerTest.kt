...
class DealerTest {
    ...
    fun `딜러는 16점을 초과하면 카드를 받을 수 없다`() {
        val dealer = Dealer()
        val card1 = Card(SuitType.HEART, NumberType.TEN)

        dealer.hit(card1)
        assertThat(dealer.canHit()).isTrue

        val card2 = Card(SuitType.HEART, NumberType.SEVEN)
        dealer.hit(card2)
    ...
    fun `딜러는 플레이어들과의 점수를 비교한 GameResult를 만들 수 있다`() {
        val cards1 = Cards.from(
            listOf(
                Card(suitType = SuitType.HEART, numberType = NumberType.QUEEN),
                Card(suitType = SuitType.HEART, numberType = NumberType.SEVEN)
            )
        )
        ...
        val cards2 = Cards.from(
            listOf(
                Card(suitType = SuitType.HEART, numberType = NumberType.TWO),
                Card(suitType = SuitType.HEART, numberType = NumberType.THREE)
            )
        )

        val cards3 = Cards.from(
            listOf(
                Card(suitType = SuitType.HEART, numberType = NumberType.TEN),
                Card(suitType = SuitType.HEART, numberType = NumberType.TEN)
            )
        )
    ...
    fun `딜러가 플레이어보다 점수가 높으면 승리한다`() {
        val dealer = Dealer()
        dealer.hit(Card(SuitType.HEART, NumberType.TEN))
        dealer.hit(Card(SuitType.HEART, NumberType.TEN))

        val player = Player.of(name = Name.from("플레이어"))
        player.hit(Card(SuitType.HEART, NumberType.TEN))
        player.hit(Card(SuitType.HEART, NumberType.TWO))
    ...
    fun `딜러가 Bust면 다른 플레이어가 Bust라도 무조건 패배한다`() {
        val dealer = Dealer()
        dealer.hit(Card(SuitType.HEART, NumberType.TEN))
        dealer.hit(Card(SuitType.HEART, NumberType.SIX))
        dealer.hit(Card(SuitType.HEART, NumberType.TEN))
