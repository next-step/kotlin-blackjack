...
class CardTest : StringSpec({
    ...
    "카드 모양은 Spade, Hart, Diamond, Clover 4개의 모양이 있다" {
        ...
        card.cardShape shouldBeIn Card.CARD_SHAPE.values()
    }
    ...
    "카드 숫자 중 1은 Ace로 표시한다" {
        val card = Card(Card.CARD_SHAPE.Clover, CardNumber.Ace())

        card.number.name shouldBe "A"
    }

    "카드 숫자 중 11은 Jack, 12는 Queen, 13은 King 으로 표시한다" {
        val cardJack = Card(Card.CARD_SHAPE.Clover, CardNumber.Jack())
        val cardQueen = Card(Card.CARD_SHAPE.Clover, CardNumber.Queen())
        val cardKing = Card(Card.CARD_SHAPE.Clover, CardNumber.King())
