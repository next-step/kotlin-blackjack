...
internal class GameDealerTest {
    ...
    fun `딜러는 게임 시작 전 카드덱을 셔플 한다`() {
        ...
        val gameDealer = GameDealer(deck).apply { shuffle() }
        assertThat(gameDealer.deck.cards).isEqualTo(deck.cards)
    }
    ...
    fun `딜러는 플레이어에게 카드 한장을 전달 할 수 있다`() {
        ...
        val gameDealer = GameDealer(CardDeckImpl(cardDeck))
        assertThat(gameDealer.deliverCard()).isEqualTo(firstCard)
        assertThat(gameDealer.deck.size).isEqualTo(resultCount)
    }
    ...
    fun `Dealer 게임 시작 전 2개의 카드를 받지 않으면 에러가 발생한다`(cards: List<Card>) {
        ...
        Assertions.assertThatExceptionOfType(IllegalArgumentException::class.java)
            .isThrownBy { gameDealer.readyToPlay(cards) }
    }

    @ParameterizedTest
    @MethodSource("provideHitCard")
    fun `Dealer 히트를 외치면 카드 한장을 더 받는다`(initialCards: List<Card>, hitCard: Card) {
        val gameDealer = GameDealer().apply {
            readyToPlay(initialCards)
            hit(hitCard)
        }
        assertThat(gameDealer.cards.value.last()).isEqualTo(hitCard)
    }
    ...
    fun `Dealer 카드 합산이 21 초과 burst 상태가 되어 게임을 더이상 참가할 수 없다`(initialCards: List<Card>, hitCard: Card) {
        ...
        assertThat(gameDealer.burst()).isTrue
    }
    ...
    fun `Dealer 카드 합산이 21 이상일 떄 burst 상태가 되어 게임을 계속 할 수있다`(initialCards: List<Card>, hitCard: Card) {
        ...
        assertThat(gameDealer.burst()).isFalse
    }
    ...
    fun `Dealer 카드가 2장이고 합산이 21이면 블랙잭 완성`() {
        ...
        val gameDealer = GameDealer(cards = cards)
        assertThat(gameDealer.blackjack()).isTrue
    }
    ...
    fun `Dealer 카드가 2장 초과이고 합산이 21이면 블랙잭 완성이 아님`() {
        ...
        val gameDealer = GameDealer(cards = cards)
        assertThat(gameDealer.blackjack()).isFalse
    }
    ...
    fun `Dealer 카드 합이 17이상이면 stay 상태로 게임을 중단한다`() {
        ...
        val gameDealer = GameDealer().apply { readyToPlay(cards) }
        assertThat(gameDealer.stay()).isTrue
    }
    ...
    fun `Dealer 카드 합이 17이상이면 stay 상태가 아니므로 카드를 추가로 받을 수 있다`() {
        ...
        val gameDealer = GameDealer().apply { readyToPlay(cards) }
        assertThat(gameDealer.stay()).isFalse
    }
