...
class CardDeckTest {
    ...
    fun `카드 덱이 52장의 카드로 이루어져 있지 않을 경우 IllegalArgumentException 을 반환한다`() {
        val cardsWithSizeLessThanFiftyTwo = (1..51).map { Card.Jack(CardSuit.CLOVER) }
        val cardsWithSizeMoreThanFiftyTwo = (1..53).map { Card.Jack(CardSuit.CLOVER) }
    ...
    fun `카드 덱에 중복되는 카드가 있을 경우 IllegalArgumentException 이 발생한다`() {
        ...
        val cardsWithDuplicates = (1..52).map { card }
