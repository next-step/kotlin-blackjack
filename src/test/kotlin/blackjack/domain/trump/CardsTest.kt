...
import blackjack.fixture.CardFixture.Companion.CLUB_FOUR
...
class CardsTest {
    ...
    fun `ACE를 포함한 카드덱의 점수 총합이 11점 이하면 ACE를 11점으로 계산한다`() {
        // given
        val cards = Cards(listOf(HEART_ACE, SPADE_THREE, CLUB_FOUR))
        val expected = 18
    ...
    fun `ACE를 포함한 카드덱의 점수 총합이 11점 초과면 ACE를 1점으로 계산한다`() {
        // given
        val cards = Cards(listOf(HEART_ACE, SPADE_THREE, CLUB_FOUR, DIAMOND_KING))
        val expected = 18
    ...
    fun `카드를 추가할 수 있다`() {
        // given
        val before = Cards(listOf(HEART_ACE, SPADE_THREE, CLUB_FOUR))
        ...
        assertAll(
            ...
            { assertThat(after.values).contains(DIAMOND_KING, HEART_ACE, SPADE_THREE, CLUB_FOUR) },
