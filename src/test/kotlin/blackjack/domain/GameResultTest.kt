...
class GameResultTest {
    ...
    fun `참가자 배팅 10000, 딜러가 bust`() {
        ...
        val players = Player("molly", 10000, drawable = GameView)
        BlackJackGame.of(
            dealer,
            listOf(players),
            MockCardListDeck(
    ...
    fun `플레이어 3명 모두 동점일 때`() {
        ...
        val dealer = Dealer("딜러")
        val players = playerNames.map { Player(it, 10000, drawable = GameView) }
    ...
    fun `플레이어가 블랙잭일때`() {
        val dealer = Dealer("딜러")
        val player = Player("molly", 10000, drawable = GameView)
