...
class PlayerTest {
    ...
    fun `누적 점수 + Ace 합이 21이 넘을것 같으면 Ace는 1로 계산한다`() {
        assertThat(
            Player(cards = listOf(Card(Suit.DIA, Denomination.KING), Card(Suit.DIA, Denomination.QUEEN), Card(Suit.DIA, Denomination.ACE))).score()
        ).isEqualTo(21)

        assertThat(
            Player(cards = listOf(Card(Suit.DIA, Denomination.KING), Card(Suit.DIA, Denomination.FOUR), Card(Suit.DIA, Denomination.ACE))).score()
        ).isEqualTo(15)

        assertThat(
            Player(
                cards = listOf(
                    Card(Suit.DIA, Denomination.KING),
                    Card(Suit.DIA, Denomination.FOUR),
                    Card(Suit.DIA, Denomination.FIVE),
                    Card(Suit.DIA, Denomination.ACE)
                )
            ).score()
        ).isEqualTo(20)
    }
