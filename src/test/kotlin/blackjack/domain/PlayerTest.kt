...
class PlayerTest : StringSpec({
    "플레이어는 이름과 카드리스트, 덱 완성 여부를 갖고 있다." {
        ...
        val player = Player(UserCards("Lee", cards))

        player.name() shouldBe "Lee"
        player.cards() shouldBe cards
        player.isDeckComplete() shouldBe false
    }
    ...
    "플레이어는 카드를 추가로 받을 수 있다." {
        ...
        player.addCard(Card(Denomination.ACE, Suit.SPADE))
    ...
    "플레이어의 덱을 완성 시킬 수 있다." {
        ...
        player.deckComplete()
        player.isDeckComplete() shouldBe true
    }
})
