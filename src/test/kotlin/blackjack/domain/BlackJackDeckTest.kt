...
class BlackJackDeckTest {
    ...
    fun `카드는 에이스(1,11)부터 2~10 JACK(10) QUEEN(10) KING(10)이 있다`() {
        val blackJackCard =
            BlackJackDeck(mutableListOf(BlackJackCard(BlackJackCardShape.HEART, BlackJackCardNumber.ACE))).draw()
    ...
    fun `중복된 카드는 지급될수 없다`() {
        val blackJackDeck =
            BlackJackDeck(mutableListOf(BlackJackCard(BlackJackCardShape.HEART, BlackJackCardNumber.ACE)))
        blackJackDeck.draw()
