...
internal class EarningRatesTest {
    @Test
    internal fun `딜러가 블랙잭이 아닌경우 블랙잭인 플레이어는 베팅금액의 150%를 수익으로 갖는다`() {
        ...
        val dealer = Dealer()
        val earningRate = EarningRates.getEarningRate(dealer, player)

        assertThat(earningRate.getEarningRate(dealer, player)).isEqualTo(1.5, Offset.offset(0.01))
    }
    ...
    internal fun `딜러가 블랙잭인 경우 블랙잭인 플레이어는 베팅금액의 100%를 수익으로 갖는다`() {
        ...
        assertThat(earningRate.getEarningRate(dealer, player)).isEqualTo(1.0, Offset.offset(0.01))
    }
    ...
    internal fun `딜러와 플레이어 둘 다 버스트가 아닌 경우, 플레이어의 점수가 더 높으면 플레이어가 베팅금액의 100%를 수익으로 갖는다`() {
        ...
        val dealer = Dealer()
        dealer.addCard(Card(Denomination.TEN, Suit.HEART))
        dealer.addCard(Card(Denomination.TWO, Suit.CLOVER))
        val earningRate = EarningRates.getEarningRate(dealer, player)
    ...
    internal fun `딜러와 플레이어 둘 다 버스트가 아닌 경우, 딜러의 점수가 더 높으면 플레이어가 베팅금액의 -100%를 수익으로 갖는다`() {
        ...
        val dealer = Dealer()
        dealer.addCard(Card(Denomination.TEN, Suit.HEART))
        dealer.addCard(Card(Denomination.SEVEN, Suit.CLOVER))
        val earningRate = EarningRates.getEarningRate(dealer, player)
