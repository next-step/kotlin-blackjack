...
class DealerTest : StringSpec({
    "딜러의 카드 포인트가 17보다 작으면 카드를 더 받는다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_10, Card.DIAMOND_6))
    ...
    "딜러의 카드 포인트가 17보다 크거나 같으면 카드를 더 받지 않는다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_10, Card.DIAMOND_7))
        val cardDeck = CardDeck()
    ...
    "딜러의 카드 포인트가 10이고 카드덱이 2, 3, 4, 5 순서라면 17보다 크거나 같을 때 까지 카드를 세 장 더 받는다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_4, Card.DIAMOND_6))
    ...
    "딜러가 플레이어보다 점수가 낮을 때 결과는 딜러의 수익은 -10000 플레이어의 수익은 10000 이다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_6, Card.DIAMOND_7))
        val players =
            listOf(Player("harris", BettingAmount(10000)).firstTurn(mutableSetOf(Card.CLOVER_10, Card.CLOVER_7)))
        //when
    ...
    "딜러가 플레이어보다 점수가 같을 때 결과는딜러의 수익은 0 플레이어의 수익은 0 이다." {
        ...
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_6, Card.DIAMOND_7))
        val players =
            listOf(Player("harris", BettingAmount(10000)).firstTurn(mutableSetOf(Card.CLOVER_6, Card.CLOVER_7)))
        //when
        val result = dealer.result(players)
    ...
    "딜러가 플레이어보다 점수가 높을 때 결과 딜러의 수익은 10000 플레이어의 수익은 -10000 이다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_8, Card.DIAMOND_7))
        val players =
            listOf(Player("harris", BettingAmount(10000)).firstTurn(mutableSetOf(Card.CLOVER_6, Card.CLOVER_7)))
        //when
        val result = dealer.result(players)
    ...
    "(플레이어 다수) 딜러 19, 플레이어1 bust 베팅 10000, 플레이어2 블랙잭 베팅 20000, 플레이어3 19 베팅 30000 일 때 딜러는 상금이 -20000원이다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_10, Card.DIAMOND_9))
        val players =
            listOf(
                Player(
                    "harris", BettingAmount(10000)
                ).firstTurn(mutableSetOf(Card.CLOVER_6, Card.CLOVER_7, Card.CLOVER_Q)),
                Player("hurris", BettingAmount(20000)).firstTurn(mutableSetOf(Card.CLOVER_A, Card.CLOVER_J)),
                Player(
                    "horris", BettingAmount(30000)
                ).firstTurn(mutableSetOf(Card.CLOVER_5, Card.CLOVER_8, Card.DIAMOND_6)),
            )
        //when
        val result = dealer.result(players)
    ...
    "(플레이어 다수) 딜러 bust, 플레이어1 bust 베팅 10000, 플레이어2 bust 20000 일 때 딜러의 상금은 30000원이다." {
        //given
        val dealer = Dealer().firstTurn(mutableSetOf(Card.DIAMOND_6, Card.DIAMOND_7, Card.DIAMOND_K))
        val players =
            listOf(
                Player(
                    "harris", BettingAmount(10000)
                ).firstTurn(mutableSetOf(Card.CLOVER_J, Card.CLOVER_4, Card.CLOVER_8)),
                Player(
                    "hurris", BettingAmount(20000)
                ).firstTurn(mutableSetOf(Card.CLOVER_K, Card.CLOVER_6, Card.CLOVER_10)),
            )
        //when
        val result = dealer.result(players)
