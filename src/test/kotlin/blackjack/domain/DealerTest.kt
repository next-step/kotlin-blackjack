...
class DealerTest {
    ...
    fun `딜러는 점수가 17 이상이면 더 이상 카드를 받지 못한다`() {
        ...
        assertThrows<ScoreOverflowException> {
            dealer.takeCards(Card(CardSymbol.SPADE, CardNumber.TWO))
        }
    ...
    fun `상대 플레이어와 동점일 경우 패한다`() {
        val dealer = Dealer()
        dealer.takeCards(Card(CardSymbol.SPADE, CardNumber.KING))

        val player = Player("test")
        player.takeCards(Card(CardSymbol.CLOVER, CardNumber.KING))
