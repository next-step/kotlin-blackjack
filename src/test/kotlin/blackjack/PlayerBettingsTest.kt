...
class PlayerBettingsTest {
    ...
    private lateinit var gamePlayer1: GamePlayer
    private lateinit var gamePlayer2: GamePlayer
    ...
    internal fun setUp() {
        ...
        gamePlayer1 = GamePlayer(name = Name.from("플레이어1"), gamePlayerState1)

        val gamePlayerCards2 = Cards(listOf(CARD_HEART_TWO, CARD_HEART_FOUR))
        val gamePlayerState2 = Stay(gamePlayerCards2)
        gamePlayer2 = GamePlayer(name = Name.from("플레이어2"), gamePlayerState2)

        players = Players.from(listOf(gamePlayer1, gamePlayer2))
    }
