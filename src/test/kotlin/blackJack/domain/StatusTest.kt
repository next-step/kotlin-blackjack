...
class StatusTest {
    ...
    @Test
    fun `점수가 21 이고 카드 갯수가 2개이면 BLACKJACK 상태를 반환`() {
    ...
    @Test
    fun `status 가 HIT 이면 Exception 이 발생하지 않는다`() {
        Status.validationAddCard(HIT)
    }

    @Test
    fun `status 가 BUST 이면 Exception 발생`() {
    ...
    @Test
    fun `status 가 STAND 이면 Exception 발생`() {
        assertThrows<IllegalArgumentException> {
    ...
    @Test
    fun `status 가 BLACKJACK 이면 Exception 발생`() {
        assertThrows<IllegalArgumentException> {
