...
class PlayerResultTest {
    ...
    fun `플레이어가 버스터인 경우 플레이어는 패배`() {
        // given
        val player = Player.of("김형준").apply {
            this.receiveCard(Card(Suit.HEARTS, Denomination.QUEEN))
            this.receiveCard(Card(Suit.SPADES, Denomination.KING))
            this.receiveCard(Card(Suit.SPADES, Denomination.NINE))
            this.receiveCard(Card(Suit.SPADES, Denomination.FOUR))
        }
        val dealer = Dealer().apply {
            this.receiveCard(Card(Suit.HEARTS, Denomination.ACE))
            this.receiveCard(Card(Suit.SPADES, Denomination.KING))
