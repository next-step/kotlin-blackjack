...
class BlackjackGameTest {
    ...
    fun `정상적인 카드팩을 받고, 게임에 주입되었을 때, 예외 검증을 통과한다`() {
        ...
        // when : 게임에 카드를 주입한다.
        val actual = runCatching { BlackjackGame(cards, defaultPlayers) }.exceptionOrNull()
    ...
    fun `플레이어 2명과 카드팩으로 구성되고, 게임이 시작될 때, 각 플레이어는 카드 2장을 받는다`() {
        ...
        // when : 게임이 시작된다.
        BlackjackGame(cards, playerGroup)
    ...
    fun `게임이 시작되고, 플레이어가 카드 받기를 요청할 때, 플레이어는 카드를 받아 저장한다`() {
        ...
        val cards = Cards(CardDeck.cards)
        val playerGroup = PlayerGroup(listOf(Player("PoPo"), Player("OYJ")))
        val game = BlackjackGame(cards, playerGroup)

        // when : 저장 여부 확인
        game.hit()
        val currentPlayer = playerGroup.getCurrentPlayer()
        val expectCard = CardDeck.cards[0]
